...
export const loginApi = {
  ...
  signIn: async (data: LoginRequestModel): Promise<ClientResponseModel[]> => {
    const response = await axiosInstance.post<ClientResponseModel[]>(API_ROUTES.LOGIN.SIGN_IN, data)
  ...
  resetPassword: async (data: LoginRequestModel): Promise<boolean> => {
    const response = await axiosInstance.post<boolean>(API_ROUTES.LOGIN.RESET_PASSWORD, data)
    return response.data
  },
  ...
  getToken: async (data: LoginRequestModel): Promise<string> => {
    const response = await axiosInstance.post<string>(API_ROUTES.LOGIN.GET_TOKEN, data)
    return response.data
  },
}
