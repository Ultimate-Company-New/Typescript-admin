...
export const productApi = {
  ...
  getProductById: async (productId: number): Promise<unknown> => {
    const response = await axiosInstance.get<unknown>(
      `${API_BASE_URL}/getProductDetailsById/${productId}`
    );
    return response.data;
  },
  ...
  toggleProduct: async (productId: number): Promise<unknown> => {
    const response = await axiosInstance.delete<unknown>(
      `${API_BASE_URL}/toggleDeleteProduct/${productId}`
    );
    return response.data;
  },
   ...
   * Uses the batches endpoint with selectedIds field for efficient filtering.
   * @param productIds - Array of product IDs to fetch
   * @returns Array of product details
   */
  getProductsByIds: async (productIds: number[]): Promise<unknown[]> => {
   ...
   */
  getProductStockAtLocationsByProductId: async (
    productId: number
