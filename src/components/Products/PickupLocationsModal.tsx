...
const PickupLocationsModal = ({ open, onClose, locations, productTitle }: PickupLocationsModalProps) => {
  return (
    <Modal
      open={open}
      onClose={onClose}
      aria-labelledby="pickup-locations-modal-title"
    >
      <Box
        sx={{
          position: 'absolute',
          ...
          {locations.length === 0 ? (
            ...
            <Grid container spacing={2}>
              {locations.map((item, index) => {
                ...
                const locationName = location.addressNickName || item.addressNickName || item.locationName || `Location ${index + 1}`
                    ...
                    >
                      <CardContent>
                        ...
                        {(() => {
                          ...
                          const hasAddress = nameOnAddress || streetAddress || streetAddress2 || streetAddress3 || city || state || postalCode || country || phoneOnAddress || emailOnAddress
                              ...
                              {hasAddress ? (
                                <Typography variant="body2">
                                  ...
                                  {phoneOnAddress && (
                                    <>
                                      <br />
                                      📞 <a href={`tel:${phoneOnAddress}`} style={{ color: 'inherit', textDecoration: 'none' }}>{phoneOnAddress}</a>
                                      <br />
                                    </>
                                  )}
                                  {emailOnAddress && (
                                    <>
                                      ✉️ <a href={`mailto:${emailOnAddress}`} style={{ color: 'inherit', textDecoration: 'none' }}>{emailOnAddress}</a>
                                    </>
                                  )}
                                </Typography>
                              ) : (
                        ...
                        {(item.contactPhone || item.contactEmail) && (
                          <Box>
                            ...
                            {item.contactPhone && (
                              <Typography variant="body2">
                                📞 <a href={`tel:${item.contactPhone}`} style={{ color: 'inherit', textDecoration: 'none' }}>{item.contactPhone}</a>
                              </Typography>
                            )}
                            {item.contactEmail && (
                              <Typography variant="body2">
                                ✉️ <a href={`mailto:${item.contactEmail}`} style={{ color: 'inherit', textDecoration: 'none' }}>{item.contactEmail}</a>
                              </Typography>
                            )}
                          </Box>
                        )}
