...
import shippingApi, { type OrderOptimizationRequest, type OrderOptimizationResponse } from '../../../api/shippingApi'
...
import ShippingOptimizationModal, { type SelectedShippingResult } from './ShippingOptimizationModal'
...
const ProductItemsSection = ({
  ...
  const [optimizationResult, setOptimizationResult] = useState<OrderOptimizationResponse | null>(null)
   ...
   * but the form uses PurchaseOrderProductItemForm (extended with allocations, fees, etc.).
   * This cast allows us to work with the extended structure internally while maintaining
   * compatibility with the parent component's API model.
   */
  ...
  const handleOpenPicker = useCallback(() => {
    if (!hasValidDeliveryAddress) {
      toast.warning('Please fill in the delivery address (City, State, and Postal Code) before adding products')
      return
    }
    setPickerOpen(true)
  ...
  const handleProductSelect = useCallback(
    (product: PurchaseOrderProductItemForm) => {
      onProductItemsChange([...formProductItems, product] as unknown as PurchaseOrderProductItem[])
    },
   ...
   * Also resets shipping calculation state since removing products invalidates
   * previous shipping calculations.
   *
   * @param {number} productId - ID of the product to remove
   */
  const handleRemoveProduct = useCallback(
    (productId: number) => {
      onProductItemsChange(formProductItems.filter((item) => item.productId !== productId) as unknown as PurchaseOrderProductItem[])
      // Reset shipping when products change - previous calculations are no longer valid
  ...
  const handleCalculateShipping = useCallback(async () => {
    if (!canCalculateShipping || !deliveryAddress?.postalCode) {
      toast.warning('Please add products and set delivery address before calculating shipping')
      return
    }
   ...
   */
  const handleShippingConfirm = useCallback((result: SelectedShippingResult) => {
    ...
    const allocations: ProductItemsSectionProps['shippingAllocations'] = result.optimizationResult.shipments.map((shipment) => {
      const selectedCourier = result.courierSelections.get(shipment.pickupLocation?.pickupLocationId || 0)
      return {
        pickupLocationId: shipment.pickupLocation?.pickupLocationId || 0,
        locationName: shipment.pickupLocation?.addressNickName || '',
     ...
     */
    const updatedProductItems = formProductItems.map((item) => ({
      ...item,
      pickupAllocations: productAllocationsMap[item.productId] || [],
    }))

    onProductItemsChange(updatedProductItems as unknown as PurchaseOrderProductItem[])
    onShippingChange(allocations as ProductItemsSectionProps['shippingAllocations'], result.totalShippingCost)
   ...
   */
  const handleCalculateCustom = useCallback(async (customAllocations: Record<number, Record<number, number>>) => {
   ...
   */
  const totalShippingCost = shippingAllocations.reduce(
   ...
   */
  const grandTotals = formProductItems.reduce(
    (acc, item) => {
      ...
      if (item.discount && item.discount > 0) {
        if (item.isDiscountPercent) {
          ...
          discountAmount = item.quantity * originalPrice * item.discount / 100
        } else {
      ...
      const itemDiscount = (item.totalDiscount && item.totalDiscount > 0) ? item.totalDiscount : discountAmount
      ...
      return {
        ...
        // Subtotal = sum of item subtotals (after discount) or grossAmount if not set
        subtotal: acc.subtotal + (item.subtotal || grossAmount),
        ...
        total: acc.total + (item.grandTotal || 0),
      }
    },
    { grossSubtotal: 0, subtotal: 0, discount: 0, packaging: 0, shipping: 0, total: 0 }
  )

  return (
    <Paper className={styles['add-purchase-order-page__section']}>
      <Subheader label="Products" className={styles['add-purchase-order-page__section-title']} />
      ...
      <ProductPickerModal
        ...
        onProductSelect={handleProductSelect as unknown as (product: PurchaseOrderProductItem) => void}
      ...
      <ShippingOptimizationModal
        ...
        productItems={formProductItems.map((item: PurchaseOrderProductItemForm): ProductItemForAllocation => ({
      ...
      <Box className={styles['product-items-section__product-list-container']}>
        {productItems.length === 0 ? (
          <Box className={styles['product-items-section__empty-state']}>
            <ProductIcon className={styles['product-items-section__empty-icon']} />
          ...
          <Box className={styles['product-items-section__product-list']}>
              ...
              return (
                <Box key={item.productId}>
                  <ProductItemCard
                    ...
                    isView={isView}
                    disabled={disabled}
                    onToggleExpand={() => setExpandedProductId(isExpanded ? null : item.productId)}
                    onRemove={() => handleRemoveProduct(item.productId)}
                  />
                  <Collapse in={isExpanded}>
                    <ProductDetailsExpanded item={item} />
                  </Collapse>
                </Box>
              )
            })}
