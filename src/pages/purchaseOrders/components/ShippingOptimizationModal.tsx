...
import productApi, { type ProductStockByLocation } from '../../../api/productApi'
import {
  type CourierOption,
 ...
 */
const CourierList = ({ couriers, selectedCourier, onCourierSelect }: CourierListProps): JSX.Element => {
  /**
  ...
  return (
    <Box className={styles['shipping-optimization-modal__courier-list-container']}>
      {/**
      ...
      {couriers.map((courier, index) => {
         ...
         */
        const isSelected = selectedCourier?.courierCompanyId === courier.courierCompanyId
         ...
         */
        const isMetadataExpanded = expandedMetadata === courier.courierCompanyId

        return (
          <Paper
            key={courier.courierCompanyId}
            variant="outlined"
            className={`${styles['shipping-optimization-modal__courier-paper']} ${isSelected ? styles['shipping-optimization-modal__courier-paper--selected'] : ''}`}
          >
            {/**
             ...
             */}
            <Box
              className={styles['shipping-optimization-modal__courier-main-info']}
              onClick={() => onCourierSelect(courier)}
            >
              <Box className={styles['shipping-optimization-modal__courier-content']}>
                {/**
                ...
                <RadioInput
                  ...
                  size="small"
                  color="success"
                  className={styles['shipping-optimization-modal__courier-radio']}
                  onChange={() => onCourierSelect(courier)}
                />
                  <Box className={styles['shipping-optimization-modal__courier-details']}>
                    {/**
                     ...
                     */}
                    <Box className={styles['shipping-optimization-modal__courier-header-row']}>
                      <BodyText variant="body2" className={styles['shipping-optimization-modal__courier-name']}>
                        {courier.courierName}
                      </BodyText>
                      {index === 0 && (
                        <Chip label="Cheapest" size="small" color="success" className={styles['shipping-optimization-modal__courier-cheapest-chip']} />
                      )}
                    </Box>
                     ...
                     */}
                    <Box className={styles['shipping-optimization-modal__courier-mode-row']}>
                      <Chip
                        icon={courier.isSurface ? <ShippingIcon className={styles['shipping-optimization-modal__courier-mode-icon']} /> : <AirIcon className={styles['shipping-optimization-modal__courier-mode-icon']} />}
                        label={courier.isSurface ? 'Surface' : 'Air'}
                        size="small"
                        variant="outlined"
                        color={courier.isSurface ? 'warning' : 'info'}
                        className={styles['shipping-optimization-modal__courier-mode-chip']}
                      />
                    </Box>
                     ...
                     */}
                    <Box className={styles['shipping-optimization-modal__courier-price-row']}>
                      <Subheader variant="h6" label={`₹${courier.rate.toLocaleString('en-IN')}`} className={styles['shipping-optimization-modal__courier-price']} />
                      <SecondaryFont variant="body2" className={styles['shipping-optimization-modal__courier-delivery']}>
                     ...
                     */}
                    <Box
                      className={styles['shipping-optimization-modal__courier-metadata-toggle']}
                      onClick={(e) => {
                        e.stopPropagation() // Prevent courier selection when clicking metadata toggle
                        setExpandedMetadata(isMetadataExpanded ? null : courier.courierCompanyId)
                      }}
                    >
                      <CodeIcon className={styles['shipping-optimization-modal__courier-metadata-icon']} />
                      <SecondaryFont variant="caption" className={styles['shipping-optimization-modal__courier-metadata-link']}>
                        {isMetadataExpanded ? '▲ Hide Shiprocket Response' : '▼ View Shiprocket Response'}
                      </SecondaryFont>
                    </Box>
                  </Box>
                </Box>
              ...
              <Collapse in={isMetadataExpanded}>
                <Box className={styles['shipping-optimization-modal__courier-metadata-content']}>
                  <BodyText component="pre" className={styles['shipping-optimization-modal__courier-metadata-pre']}>
 ...
 * - Weight-split detection (shows "Shipment X of Y" label)
 *
 * Layout:
 ...
 * - Each shipment gets a label: "Shipment 1 of 3"
...
const ShipmentDetails = ({
  ...
  const shipmentLabel = isWeightSplit
    ? `Shipment ${shipmentIndex} of ${totalShipmentsFromLocation}`
    : null
  ...
  return (
    <Card
      ...
      style={{ borderColor: !isValid ? '#d32f2f' : isExpanded ? '#1976d2' : undefined, borderWidth: isExpanded ? 2 : 1, transition: 'all 0.2s', opacity: isValid ? 1 : 0.5 }}
    >
      <CardContent className={`${styles['shipping-optimization-modal__shipment-card-content']} ${isExpanded ? styles['shipping-optimization-modal__shipment-card-content--expanded'] : ''}`}>
        ...
        <ShipmentHeader
          shipment={shipment}
          selectedCourier={selectedCourier as CourierOption | undefined}
          ...
          <Box className={styles['shipping-optimization-modal__shipment-expanded-details']}>
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} md={5}>
                <Subheader variant="subtitle2" label="Select Courier" className={styles['shipping-optimization-modal__shipment-couriers-header']}>
                  <ShippingIcon fontSize="small" color="success" />
                </Subheader>
                {shipment.availableCouriers.length === 0 ? (
                  <Paper variant="outlined" className={styles['shipping-optimization-modal__shipment-no-couriers-paper']}>
                    <ShippingIcon color="warning" className={styles['shipping-optimization-modal__shipment-no-couriers-icon']} />
                    <BodyText variant="body2" className={styles['shipping-optimization-modal__shipment-no-couriers-text']}>
                      No couriers available for this route
                    </BodyText>
                  </Paper>
                ) : (
                  <CourierList
                    couriers={shipment.availableCouriers}
...
const CustomAllocationTab = ({
   ...
   */
  const [stockByProduct, setStockByProduct] = useState<Record<number, ProductStockState>>({})
   ...
   */
  const [allocations, setAllocations] = useState<Record<number, Record<number, number>>>({})
   ...
   */
  const [addedLocationIds, setAddedLocationIds] = useState<Record<number, number[]>>({})
   ...
   */
  const handleAllocationChange = (productId: number, locationId: number, quantity: number) => {
    setAllocations((prev) => {
      /**
  ...
  const handleRemoveLocation = (productId: number, locationId: number) => {
    setAddedLocationIds((prev) => {
      const productLocations = prev[productId] || []
      return { ...prev, [productId]: productLocations.filter(id => id !== locationId) }
    })
  ...
  if (loadingStock && !isCalculating) {
    return (
      <Box className={styles['shipping-optimization-modal__custom-allocation-loading']}>
        <CircularProgress size={48} />
        <Box className={styles['shipping-optimization-modal__custom-allocation-loading-text']}>
  ...
  if (productItems.length === 0) {
    return (
      <Box className={styles['shipping-optimization-modal__custom-allocation-empty']}>
        <ProductIcon className={styles['shipping-optimization-modal__custom-allocation-empty-icon']} />
  ...
  const handleFillTestAllocations = () => {
    ...
    for (const product of productItems) {
       ...
       */
      const sortedLocations = [...locations].sort((a, b) => (b.availableStock || 0) - (a.availableStock || 0))
      ...
      const loc1 = sortedLocations.find((loc) => (loc.availableStock || 0) >= qty1)
      if (!loc1) {
        toast.warning(`Could not find location with enough stock for ${product.title}`)
        continue
      }
      ...
      const loc2 = sortedLocations.find(
        (loc) => loc.pickupLocationId !== loc1.pickupLocationId && (loc.availableStock || 0) >= qty2
      )
      ...
      if (loc2) {
         ...
         */
        newAddedLocationIds[product.productId] = [loc1.pickupLocationId, loc2.pickupLocationId]
        ...
        if ((loc1.availableStock || 0) >= requiredQty) {
          ...
          toast.warning(`Not enough stock in any single location for ${product.title}`)
        }
      }
    }
  ...
  return (
    <Box>
      ...
      {customResult?.shipments?.length ? (
        <Box className={styles['shipping-optimization-modal__custom-allocation-results-container']}>
          <Box className={styles['shipping-optimization-modal__custom-allocation-results-list']}>
            {/**
            ...
            {(() => {
               ...
               */
              const shipmentCountByLocation = customResult.shipments.reduce((acc, s) => {
              ...
              return customResult.shipments.map((shipment, index) => {
                ...
                const totalFromLocation = shipmentCountByLocation[locationId] || 1
                 ...
                 */
                locationIndexTracker[locationId] = (locationIndexTracker[locationId] || 0) + 1
                const shipmentIndexForLocation = locationIndexTracker[locationId]
                ...
                const expandKey = totalFromLocation > 1 ? shipmentKey : `custom-${locationId}`

                return (
                  <ShipmentDetails
                    key={shipmentKey}
                    shipment={shipment as any}
                    selectedCourier={courierSelections.get(locationId)}
                    onCourierSelect={(courier) => onCourierSelect(locationId, courier)}
       ...
       */}
      <Box className={styles['shipping-optimization-modal__custom-allocation-fill-button-container']}>
      ...
      {productItems.map((product) => {
        ...
        return (
          <Accordion
            ...
            onChange={(_, isExpanded) => setExpandedProduct(isExpanded ? product.productId : null)}
            className={styles['shipping-optimization-modal__custom-allocation-accordion']}
          >
            <AccordionSummary
              expandIcon={<ExpandMoreIcon />}
              className={styles['shipping-optimization-modal__custom-allocation-accordion-summary']}
            >
              <Box className={styles['shipping-optimization-modal__custom-allocation-product-header']}>
                {/* Product Image */}
                {product.mainImageUrl ? (
                  <Box
                    ...
                    alt={product.title}
                    className={styles['shipping-optimization-modal__custom-allocation-product-image']}
                  />
                ) : (
                  <Box className={styles['shipping-optimization-modal__custom-allocation-product-image-placeholder']}>
                    <ProductIcon className={styles['shipping-optimization-modal__custom-allocation-product-image-placeholder-icon']} />
                  </Box>
                )}

                {/* Product Title */}
                <Box className={styles['shipping-optimization-modal__custom-allocation-product-info']}>
                  <Subheader variant="subtitle1" label={product.title} component="div" className={styles['shipping-optimization-modal__custom-allocation-product-title']} />
                  <SecondaryFont variant="body2" component="div" className={styles['shipping-optimization-modal__custom-allocation-product-quantity']}>
                ...
                <Chip
                  ...
                  color={isComplete ? 'success' : isOverAllocated ? 'error' : 'warning'}
                  size="small"
                  icon={isComplete ? <CheckIcon /> : undefined}
                  className={styles['shipping-optimization-modal__custom-allocation-status-chip']}
                />
              </Box>
            </AccordionSummary>
            ...
            <AccordionDetails>
              {stockState?.loading ? (
                <Box className={styles['shipping-optimization-modal__custom-allocation-loading-stock']}>
                  <CircularProgress size={24} />
                </Box>
              ) : stockState?.error ? (
                <BodyText className={styles['shipping-optimization-modal__custom-allocation-error-text']}>{stockState.error}</BodyText>
              ) : locations.length === 0 ? (
                <SecondaryFont className={styles['shipping-optimization-modal__custom-allocation-no-locations-text']}>No pickup locations available for this product</SecondaryFont>
              ) : (
                <Box className={styles['shipping-optimization-modal__custom-allocation-locations-list']}>
                  /**
                   ...
                   */
                  {(() => {
                    const productLocationIds = addedLocationIds[product.productId] || []
                    /**
                     ...
                     */
                    const locationMap = new Map(locations.map(loc => [loc.pickupLocationId, loc]))
                    /**
                    ...
                    const addedLocationsList = productLocationIds
                      ...
                      .filter((loc): loc is ProductStockByLocation => loc !== undefined)

                    return addedLocationsList.length > 0 ? (
                      <>
                        <SecondaryFont variant="caption" className={styles['shipping-optimization-modal__custom-allocation-selected-locations-label']}>
                          Selected Locations ({addedLocationsList.length})
                        </SecondaryFont>
                        {addedLocationsList.map((location) => {
                          const currentAllocation = allocations[product.productId]?.[location.pickupLocationId] || 0
                          ...
                          return (
                            <Paper
                              ...
                              variant="outlined"
                              className={styles['shipping-optimization-modal__custom-allocation-location-paper']}
                            >
                              {/* Location Info */}
                              <Box className={styles['shipping-optimization-modal__custom-allocation-location-info']}>
                                <Box className={styles['shipping-optimization-modal__custom-allocation-location-header']}>
                                  <LocationIcon fontSize="small" color="primary" className={styles['shipping-optimization-modal__custom-allocation-location-icon']} />
                                  <BodyText variant="body2" className={styles['shipping-optimization-modal__custom-allocation-location-name']}>
                                    {location.locationName}
                                  </BodyText>
                                </Box>
                                <SecondaryFont variant="caption" className={styles['shipping-optimization-modal__custom-allocation-location-address']}>
                                  {[location.city, location.state, location.postalCode].filter(Boolean).join(', ')}
                                </SecondaryFont>
                              </Box>

                              {/* Available Stock */}
                              <Box className={styles['shipping-optimization-modal__custom-allocation-location-stock']}>
                                <SecondaryFont variant="caption" className={styles['shipping-optimization-modal__custom-allocation-location-stock-label']}>Stock</SecondaryFont>
                                <BodyText variant="body2" className={styles['shipping-optimization-modal__custom-allocation-location-stock-value']}>
                                  {maxAllowable}
                                </BodyText>
                              </Box>
                              ...
                              <TextFieldInput
                                ...
                                onChange={(e) => {
                                  ...
                                  handleAllocationChange(product.productId, location.pickupLocationId, value)
                                }}
                                inputProps={{ min: 0 }}
                                error={maxAllowable > 0 && currentAllocation > maxAllowable}
                                className={styles['shipping-optimization-modal__custom-allocation-location-quantity-field']}
                              />
                              ...
                              <IconButton
                                size="small"
                                color="error"
                                onClick={() => handleRemoveLocation(product.productId, location.pickupLocationId)}
                              >
                        ...
                        <Divider className={styles['shipping-optimization-modal__custom-allocation-divider']} />
                      </>
                    ) : null
                  })()}
                  ...
                  <Box>
                    <TextFieldInput
                      ...
                      onChange={(e) => setLocationSearch(prev => ({ ...prev, [product.productId]: e.target.value }))}
                      InputProps={{
                        startAdornment: <LocationIcon fontSize="small" className={styles['shipping-optimization-modal__custom-allocation-search-icon']} />,
                      }}
                      className={styles['shipping-optimization-modal__custom-allocation-search-field']}
                    />
                    ...
                    {(() => {
                      ...
                      const searchTerm = (locationSearch[product.productId] || '').toLowerCase()
                      ...
                      const searchResults = locations
                        .filter(loc =>
                          ...
                          !addedSet.has(loc.pickupLocationId) && (
                            ...
                            loc.postalCode?.toLowerCase().includes(searchTerm)
                          )
                        )
                      ...
                      if (searchResults.length === 0) {
                        return (
                          <Paper variant="outlined" className={styles['shipping-optimization-modal__custom-allocation-no-available-locations-paper']}>
                            <SecondaryFont variant="body2" className={styles['shipping-optimization-modal__custom-allocation-no-available-locations-text']}>
                      ...
                      return (
                        <Paper variant="outlined" style={{ marginTop: 4, maxHeight: 250, overflow: 'auto' }}>
                          {searchResults.map((location) => (
                            <Box
                              ...
                              onClick={() => handleAddLocation(product.productId, location.pickupLocationId)}
                            >
                              <LocationIcon fontSize="small" color="primary" style={{ flexShrink: 0 }} />
                              ...
                              <Chip
                                ...
                                size="small"
                                color={(location.availableStock ?? 0) > 0 ? 'success' : 'default'}
                                variant="outlined"
                              />
                              <BlueButton size="small" variant="outlined" color="primary">
                                Add
                              </BlueButton>
                            </Box>
                          ))}
                  ...
                  {!(addedLocationIds[product.productId]?.length > 0) && (
                    <SecondaryFont variant="caption" style={{ textAlign: 'center', paddingTop: 16, paddingBottom: 16 }}>
...
const ShippingOptimizationModal = ({
   ...
   */
  const [customCourierSelections, setCustomCourierSelections] = useState<Map<number, CourierOption>>(new Map())
   ...
   */
  const [customResult, setCustomResult] = useState<OrderOptimizationResponse | null>(null)
   ...
   */
  const [customExpandedLocationId, setCustomExpandedLocationId] = useState<number | string | null>(null)
   ...
   */
  const [customAllocations, setCustomAllocations] = useState<Record<number, Record<number, number>>>({})
   ...
   * - Selects first courier (assumed to be cheapest, sorted by API)
   ...
   * - Provides sensible default for users
   ...
   */
  const initializeSelections = useCallback((result: OrderOptimizationResponse, isCustom: boolean = false) => {
    if (result.shipments) {
      const initialSelections = new Map<number, CourierOption>()
      for (const shipment of result.shipments) {
        const locationId = shipment.pickupLocation?.pickupLocationId
        if (locationId && shipment.availableCouriers.length > 0) {
          // Select the first courier by default (already sorted by rate, cheapest first)
          initialSelections.set(locationId, shipment.availableCouriers[0])
        }
      }
   ...
   */
  const handleCalculateCustomWrapper = useCallback(async (allocations: Record<number, Record<number, number>>) => {
  ...
  useEffect(() => {
    if (activeTab === 1 && customLoading && optimizationResult !== prevOptimizationResult.current && optimizationResult) {
       ...
       */
      allocationsUsedForCalculationRef.current = JSON.parse(JSON.stringify(customAllocations))
  ...
  }, [optimizationResult, activeTab, customLoading, initializeSelections, customAllocations])
   ...
   */
  const allocationsUsedForCalculationRef = useRef<Record<number, Record<number, number>> | null>(null)
  ...
  useEffect(() => {
    ...
    if (activeTab === 1 && customResult?.shipments?.length && allocationsUsedForCalculationRef.current !== null) {
      /**
       ...
       */
      const allocationsChanged = JSON.stringify(allocationsUsedForCalculationRef.current) !== JSON.stringify(customAllocations)
   ...
   * Each courier selection represents one shipment's courier.
   *
   ...
   *
   * Same as calculateTotalShipping but uses customCourierSelections instead.
  ...
  const calculateCustomTotalShipping = useCallback(() => {
    let total = 0
    for (const [, courier] of customCourierSelections) {
      total += courier.rate
    }
    return total
  }, [customCourierSelections])
  ...
  const handleConfirm = useCallback(() => {
    ...
    toast.success(
      `Shipping confirmed: ₹${(result.totalShippingCost + result.totalPackagingCost).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`
    )

    // Close modal
    onClose()
  }, [optimizationResult, courierSelections, calculateTotalShipping, onConfirm, onClose])

  // Error state
  if (optimizationResult && !optimizationResult.success) {
    return (
      <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
        <DialogTitle className={styles['shipping-optimization-modal__error-title']}>
        ...
        <DialogContent>
          <Box className={styles['shipping-optimization-modal__error-content']}>
            <BodyText className={styles['shipping-optimization-modal__error-text']} variant="body1">
              {optimizationResult.errorMessage || 'Failed to optimize order fulfillment'}
            </BodyText>
          </Box>
        </DialogContent>
        <DialogActions>
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xl" fullWidth PaperProps={{ sx: { maxHeight: '95vh' } }}>
      <DialogTitle className={styles['shipping-optimization-modal__title']}>
        <Box className={styles['shipping-optimization-modal__title-left']}>
          ...
          {optimizationResult?.shipments?.length && (
            <Chip
              label={`${optimizationResult.shipments.length} Shipment${optimizationResult.shipments.length !== 1 ? 's' : ''}`}
              size="small"
              color="primary"
            />
          )}
      ...
      <DialogContent ref={dialogContentRef} className={styles['shipping-optimization-modal__dialog-content']}>
        {/* Tab 0: System Recommended */}
        {/* Only show isLoading loader for System Recommended tab, completely ignore it for Custom tab */}
        {activeTab === 0 && (shouldShowSystemRecommendedLoader ? (
          <Box className={styles['shipping-optimization-modal__custom-allocation-loading']}>
            <CircularProgress size={48} />
            <Box className={styles['shipping-optimization-modal__custom-allocation-loading-text']}>
              <Subheader variant="h6" label="Calculating optimal shipping..." />
              <SecondaryFont variant="body2">
          ...
          <Box className={styles['shipping-optimization-modal__custom-allocation-empty']}>
            <ShippingIcon className={styles['shipping-optimization-modal__custom-allocation-empty-icon']} />
          ...
          <Box className={styles['shipping-optimization-modal__custom-allocation-results-list']}>
            ...
            {(() => {
               ...
               */
              const shipmentCountByLocation = optimizationResult.shipments.reduce((acc, s) => {
                const locId = s.pickupLocation?.pickupLocationId || 0
                acc[locId] = (acc[locId] || 0) + 1
                return acc
              }, {} as Record<number, number>)
              ...
              return optimizationResult.shipments.map((shipment, index) => {
                ...
                const expandKey = totalFromLocation > 1 ? shipmentKey : locationId
                ...
                return (
                  <ShipmentDetails
                    ...
                    shipment={shipment as any}
                    selectedCourier={courierSelections.get(locationId)}
                    onCourierSelect={(courier) => handleCourierSelect(locationId, courier as any)}
                    isExpanded={expandedLocationId === expandKey}
                    onToggleExpand={() => setExpandedLocationId((prev) => (prev === expandKey ? null : expandKey))}
                    shipmentIndex={shipmentIndexForLocation}
                    totalShipmentsFromLocation={totalFromLocation}
                  />
                )
         ...
         */}
        <Box className={styles['shipping-optimization-modal__custom-tab-container']} style={{ display: activeTab === 1 ? 'block' : 'none' }}>
          <CustomAllocationTab
            key={customTabKey}
             ...
             */
            onCourierSelect={(locationId, courier) => {
              setCustomCourierSelections((prev) => {
                const next = new Map(prev)
                next.set(locationId, courier as any)
                return next
              })
            }}
             ...
             */
            onToggleExpand={(key) => setCustomExpandedLocationId((prev) => (prev === key ? null : key))}
            /**
      ...
      <DialogActions className={styles['shipping-optimization-modal__dialog-actions']}>
        <Box>
          {/* System Recommended tab - show totals */}
          {activeTab === 0 && optimizationResult?.shipments?.length && (
            <Box className={styles['shipping-optimization-modal__totals-container']}>
              <Box className={styles['shipping-optimization-modal__totals-row']}>
                <SecondaryFont variant="body2" className={styles['shipping-optimization-modal__totals-label']}>📦 Packaging:</SecondaryFont>
                <BodyText variant="body2" className={styles['shipping-optimization-modal__totals-value']}>
                  ₹{optimizationResult.totalPackagingCost.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                </BodyText>
              </Box>
              <Box className={styles['shipping-optimization-modal__totals-row']}>
                <SecondaryFont variant="body2" className={styles['shipping-optimization-modal__totals-label']}>🚚 Shipping:</SecondaryFont>
                <BodyText variant="body2" className={styles['shipping-optimization-modal__totals-value']}>
                  ₹{calculateTotalShipping().toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                </BodyText>
              </Box>
              <Box className={styles['shipping-optimization-modal__totals-total-row']}>
                <BodyText variant="body1" className={styles['shipping-optimization-modal__totals-total-label']}>Total:</BodyText>
                <Subheader variant="h5" label={`₹${(optimizationResult.totalPackagingCost + calculateTotalShipping()).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`} className={styles['shipping-optimization-modal__totals-total-value']} />
              </Box>
            </Box>
          )}
          ...
          {activeTab === 1 && customResult?.shipments?.length && (
            <Box className={styles['shipping-optimization-modal__totals-container']}>
              <Box className={styles['shipping-optimization-modal__totals-row']}>
                <SecondaryFont variant="body2" className={styles['shipping-optimization-modal__totals-label']}>📦 Packaging:</SecondaryFont>
                <BodyText variant="body2" className={styles['shipping-optimization-modal__totals-value']}>
                  ₹{customResult.totalPackagingCost.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                </BodyText>
              </Box>
              <Box className={styles['shipping-optimization-modal__totals-row']}>
                <SecondaryFont variant="body2" className={styles['shipping-optimization-modal__totals-label']}>🚚 Shipping:</SecondaryFont>
                <BodyText variant="body2" className={styles['shipping-optimization-modal__totals-value']}>
                  ₹{calculateCustomTotalShipping().toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                </BodyText>
              </Box>
              <Box className={styles['shipping-optimization-modal__totals-total-row']}>
                <BodyText variant="body1" className={styles['shipping-optimization-modal__totals-total-label']}>Total:</BodyText>
                <Subheader variant="h5" label={`₹${(customResult.totalPackagingCost + calculateCustomTotalShipping()).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`} className={styles['shipping-optimization-modal__totals-total-value']} />
              </Box>
            </Box>
          )}
          {/* Custom tab without results - show hint */}
          {activeTab === 1 && !customResult?.shipments?.length && !customLoading && (
            <SecondaryFont variant="body2" className={styles['shipping-optimization-modal__totals-hint']}>
          ...
          {activeTab === 1 && customLoading && (
            <Box className={styles['shipping-optimization-modal__totals-loading']}>
              <CircularProgress size={20} />
              <SecondaryFont variant="body2" className={styles['shipping-optimization-modal__totals-loading-text']}>Calculating...</SecondaryFont>
            </Box>
          )}
        </Box>
        <Box className={styles['shipping-optimization-modal__dialog-actions-buttons']}>
          ...
          {activeTab === 0 && (
            <BlueButton
              ...
              disabled={!optimizationResult?.shipments?.length || shouldShowSystemRecommendedLoader}
            >
              Confirm Shipping
            </BlueButton>
          )}
          ...
          {activeTab === 1 && !customResult?.shipments?.length && (
            <BlueButton
              variant="contained"
              color="primary"
              startIcon={customLoading ? <CircularProgress size={20} color="inherit" /> : <ShippingIcon />}
...
export type { ProductItemForAllocation, SelectedShippingResult } from '../../../models'
