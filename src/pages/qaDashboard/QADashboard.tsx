...
const QADashboard = (): JSX.Element => {
          ...
          method.associatedUnitTests.forEach(test => {
            ...
            if (test.hasBeenRun && test.lastRunStatus) {
              ...
              methodResults.push({
                ...
                status: test.lastRunStatus.toLowerCase() as 'passed' | 'failed' | 'skipped',
  ...
  const saveTestRunResults = async (
    ...
    try {
      const testResults: TestResultData[] = results.map(r => ({
        ...
        status: r.status.toUpperCase() as 'PASSED' | 'FAILED' | 'SKIPPED' | 'ERROR',
  ...
  const pollTestExecution = async (
    ...
    for (let i = 0; i < MAX_POLLS; i++) {
      try {
        ...
        if (status.status === 'COMPLETED' || status.status === 'COMPLETED_WITH_FAILURES' ||
  ...
  const convertResults = (status: TestExecutionStatusModel): TestResult[] => {
    return status.results.map(r => ({
      ...
      status: r.status.toLowerCase() as 'passed' | 'failed' | 'skipped' | 'running',
  ...
  const resolveDeclaringTestClassName = (serviceName: string, methodName: string, testMethodName: string): string | null => {
    const service = services.find(s => s.serviceName === serviceName)
    const method = service?.methods?.find(m => m.methodName === methodName)
    const testInfo = method?.associatedUnitTests?.find(t => t.testMethodName === testMethodName)
    ...
    try {
      ...
      const finalStatus = await pollTestExecution(
        ...
        (status) => {
          ...
          const progress = status.progressPercentage ?? (totalTests > 0 ? (completedTests / totalTests) * 100 : 0)
          ...
          const optimisticPassed = Math.max(0, completedTests - failedTests - skippedTests)
          const passedTests = Math.min(completedTests, Math.max(backendPassed, optimisticPassed))
      ...
      const failedTests = finalStatus.failedTests ?? results.filter(r => r.status === 'failed').length
      const skippedTests = finalStatus.skippedTests ?? results.filter(r => r.status === 'skipped').length
      const passedTests = finalStatus.passedTests ?? Math.max(0, completedTests - failedTests - skippedTests)
      setServiceTestStates(prev => ({
        ...prev,
        [serviceName]: {
          ...prev[serviceName],
      ...
      void saveTestRunResults(serviceName, method.methodName, results, 'SINGLE_METHOD')
    } catch (err) {
  ...
  const runSingleTest = async (
    ...
    const service = services.find(s => s.serviceName === serviceName)
    const testClassName = declaringTestClassName || service?.testClassName || serviceName.replace('Service', 'ServiceTest')
    ...
    try {
      ...
      const results = convertResults(finalStatus)
      const result = results.find(r => r.testName === testMethodName) || results[0]

      if (result) {
        setServiceTestStates(prev => {
          const currentResults = prev[serviceName]?.methodStates[methodName]?.results ?? []
          const existingIndex = currentResults.findIndex(r => r.testName === testMethodName)
          const newResults = existingIndex >= 0
            ? [...currentResults.slice(0, existingIndex), result, ...currentResults.slice(existingIndex + 1)]
  ...
  const runAllServiceTests = async (service: QAServiceResponse) => {
    ...
    setServiceTestStates(prev => ({
      ...
      [service.serviceName]: {
        ...
        totalTests,
        completedTests: 0,
        // Reset all method states to running
        methodStates: Object.fromEntries(
          service.methods.map(m => [m.methodName, { status: 'running' as TestStatus, results: [], expanded: false }])
        ),
      },
    }))

    try {
      // Start test execution via API for entire service
      const initialStatus = await qaApi.runTests({
      ...
      const finalStatus = await pollTestExecution(
        ...
        (status) => {
          // Update UI with progress
          const completedTests = status.completedTests
          const progress = status.progressPercentage

          // Group results by method
          const resultsByMethod: { [methodName: string]: TestResult[] } = {}
          status.results.forEach(r => {
            const methodName = r.methodName || r.testMethodName.split('_')[0]
            if (!resultsByMethod[methodName]) {
          ...
          setServiceTestStates(prev => {
            const newMethodStates: MethodTestState = { ...prev[service.serviceName].methodStates }

            Object.entries(resultsByMethod).forEach(([methodName, results]) => {
              const hasFailed = results.some(r => r.status === 'failed')
              const allComplete = results.length >= (service.methods.find(m => m.methodName === methodName)?.testCount || 0)

              newMethodStates[methodName] = {
                status: allComplete ? (hasFailed ? 'failed' : 'passed') : 'running',
                results,
                expanded: prev[service.serviceName].methodStates[methodName]?.expanded || false,
              }
            })
      ...
      setServiceTestStates(prev => {
        const newMethodStates: MethodTestState = {}

        service.methods.forEach(method => {
          const results = resultsByMethod[method.methodName] || []
          const hasFailed = results.some(r => r.status === 'failed')

          newMethodStates[method.methodName] = {
            status: results.length > 0 ? (hasFailed ? 'failed' : 'passed') : 'idle',
            results,
            expanded: prev[service.serviceName].methodStates[method.methodName]?.expanded || false,
          }
        })
        ...
        return {
          ...
          [service.serviceName]: {
            ...
            isRunning: false,
            progress: 100,
            completedTests: finalStatus.completedTests,
            methodStates: newMethodStates,
          },
        }
      })
    } catch (err) {
      console.error('Error running service tests:', err)
      setServiceTestStates(prev => ({
        ...prev,
        [service.serviceName]: {
          ...prev[service.serviceName],
      ...
      setError(`Failed to run tests for ${service.serviceName}`)
    }
  }
  ...
  const getStatusIcon = (status: TestStatus) => {
    switch (status) {
      case 'passed':
        return <CheckCircleIcon className={styles['qa-dashboard__icon--passed']} />
      case 'failed':
    ...
    return (
      <Box className={styles['qa-dashboard__stats']}>
        <Paper className={styles['qa-dashboard__stat-card']}>
          <Typography variant="h3" className={styles['qa-dashboard__stat-value']}>
        ...
        <Paper className={styles['qa-dashboard__stat-card']}>
          ...
          <Typography variant="body2" color="textSecondary">Unit Tests</Typography>
        </Paper>
        <Paper className={styles['qa-dashboard__stat-card']}>
          <Typography variant="h3" className={styles['qa-dashboard__stat-value--coverage']}>
  ...
  const renderTestResult = (result: TestResult, serviceName: string, methodName: string) => (
    <Box key={result.testName} className={styles['qa-dashboard__test-result']}>
      <Box className={styles['qa-dashboard__test-result-header']}>
        {result.status === 'passed' ? (
          <CheckCircleIcon className={styles['qa-dashboard__icon--passed']} fontSize="small" />
        ) : (
          <ErrorIcon className={styles['qa-dashboard__icon--failed']} fontSize="small" />
        )}
        <Box className={styles['qa-dashboard__test-result-info']} sx={{ flex: 1 }}>
          <Typography variant="body2" className={styles['qa-dashboard__test-name']}>
            {result.testName}
          </Typography>
          {result.displayName && (
          ...
          <Tooltip title="Re-run this test">
            <IconButton
              ...
              onClick={() => runSingleTest(
                ...
                result.testName,
                result.displayName,
                resolveDeclaringTestClassName(serviceName, methodName, result.testName)
              )}
            >
      ...
      {result.status === 'failed' && result.errorMessage && (
        <Alert severity="error" className={styles['qa-dashboard__error-alert']}>
          ...
          {result.stackTrace && (
            <Typography variant="caption" component="pre" className={styles['qa-dashboard__stack-trace']}>
              {result.stackTrace}
            </Typography>
          )}
        </Alert>
  ...
  const renderMethodCard = (service: QAServiceResponse, method: MethodInfo) => {
    ...
    const runningCompleted = methodState?.completedTests ?? (methodState?.results?.length ?? 0)
    const runningProgress = methodState?.progress ?? (runningTotal > 0 ? (runningCompleted / runningTotal) * 100 : 0)

    // Calculate passed/failed counts from results
    const passedCount = methodState?.results?.filter(r => r.status === 'passed').length ?? 0
    const failedCount = methodState?.results?.filter(r => r.status === 'failed').length ?? 0

    return (
      <Paper key={method.methodName} className={styles['qa-dashboard__method-card']}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <Box sx={{ flex: 1 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              {methodState?.status && methodState.status !== 'idle' && getStatusIcon(methodState.status)}
            </Box>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            <Tooltip title={method.testCount > 0 ? 'Run all tests' : 'No tests available'}>
              <span>
                <IconButton
                  size="small"
                  color="primary"
            ...
            {method.testCount > 0 && (
              <IconButton
                size="small"
                onClick={() => toggleMethodExpanded(service.serviceName, method.methodName)}
              >
                {methodState?.expanded ? <ExpandLessIcon /> : <ExpandMoreIcon />}
              </IconButton>
            )}
          </Box>
        </Box>
        ...
        <Collapse in={methodState?.expanded && method.testCount > 0}>
          ...
          <Box className={styles['qa-dashboard__test-results']}>
            {hasResults ? (
              ...
              methodState?.results.map(result => renderTestResult(result, service.serviceName, method.methodName))
            ) : (
              ...
              method.associatedUnitTests.map((test) => {
                ...
                const isPassed = hasRun && test.lastRunStatus?.toUpperCase() === 'PASSED'
                const isFailed = hasRun && (test.lastRunStatus?.toUpperCase() === 'FAILED' || test.lastRunStatus?.toUpperCase() === 'ERROR')

                return (
                  <Box key={test.testMethodName} className={styles['qa-dashboard__test-result']}>
                    <Box className={styles['qa-dashboard__test-result-header']}>
                      {hasRun ? (
                        isPassed ? (
                          ...
                          <HourglassEmptyIcon fontSize="small" sx={{ color: 'text.disabled', mr: 1 }} />
                        )
                      ) : (
                        <HourglassEmptyIcon fontSize="small" sx={{ color: 'text.disabled', mr: 1 }} />
                      )}
                      <Box className={styles['qa-dashboard__test-result-info']} sx={{ flex: 1 }}>
                        ...
                        {hasRun && test.lastRunAt && (
                          <Typography variant="caption" color="textSecondary" sx={{ display: 'block', mt: 0.5 }}>
                            Last run: {new Date(test.lastRunAt).toLocaleString()} by {test.lastRunByUserName}
                            {test.lastRunDurationMs != null && ` • ${test.lastRunDurationMs}ms`}
                          </Typography>
                        )}
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        ...
                        <Tooltip title="Run this test">
                          <IconButton
                            size="small"
                            color="primary"
                            onClick={() => runSingleTest(service.serviceName, method.methodName, test.testMethodName, test.displayName, test.declaringTestClassName)}
                          >
                            <PlayArrowIcon fontSize="small" />
                          </IconButton>
                        </Tooltip>
  ...
  const renderServiceAccordion = (service: QAServiceResponse) => {
    ...
    if (serviceState?.methodStates) {
      Object.values(serviceState.methodStates).forEach(methodState => {
        if (methodState.results) {
          passedCount += methodState.results.filter(r => r.status === 'passed').length
          failedCount += methodState.results.filter(r => r.status === 'failed').length
        }
      })
    }
    ...
    return (
      <Accordion
        ...
        onChange={(_, isExpanded) => setExpandedService(isExpanded ? service.serviceName : false)}
          ...
          <Box sx={{ width: '100%' }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1.5 }}>
              <Box className={styles['qa-dashboard__service-info']}>
                <Typography variant="h6" className={styles['qa-dashboard__service-name']}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              {/* Left: Summary chips */}
              <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                <Chip size="small" label={`${service.totalMethods} methods`} variant="outlined" />
                <Chip
                  size="small"
                ...
                <Chip
                  ...
                  color={service.coveragePercentage >= 80 ? 'success' : service.coveragePercentage >= 50 ? 'warning' : 'error'}
                />
              </Box>
        ...
        <AccordionDetails className={styles['qa-dashboard__service-details']}>
            ...
            <Box className={styles['qa-dashboard__service-progress']}>
              <Box className={styles['qa-dashboard__progress-header']}>
                <Typography variant="body2">
                  Running tests: {serviceState.completedTests}/{serviceState.totalTests}
                </Typography>
          ...
          <Box className={styles['qa-dashboard__run-all-container']}>
            <Button
              variant="contained"
              color="primary"
              startIcon={isRunning ? <CircularProgress size={18} color="inherit" /> : <PlayCircleOutlineIcon />}
  ...
  return (
    <Container maxWidth="xl">
      <Box className={styles['qa-dashboard__container']}>
        {/* Error Alert */}
        {error && (
          <Alert severity="error" onClose={() => setError(null)} className={styles['qa-dashboard__alert']}>
            {error}
          </Alert>
        )}
          ...
          <Box className={styles['qa-dashboard__tab-body']}>
            ...
            <TabPanel value={currentTab} index="unit">
              ...
              <Box className={styles['qa-dashboard__services']}>
                <Typography variant="h6" className={styles['qa-dashboard__section-title']}>
                  Services & Unit Tests
                </Typography>
